...
$re = mysqli_query($conn, "SELECT DISTINCT u.last_name, u.first_name, u.pathronymic, r.result, r.id_test, a.answer FROM `users` u, `result` r, `answer` a WHERE u.id_user=r.id_user AND r.id_test=a.id_test AND u.id_user=$id_user AND r.id_test = '4'");

//Переменная для вывода названия теста, который не пройден
$res_error = mysqli_query($conn, "SELECT * FROM `test` WHERE id_test = '4'");
...
<body>
  <? include("menu.php"); ?>
  <div class="card">
  <a href='../../../word_user.php?id_user=<?= $id_user?>&id_test=4'>Сохранить результат курсанта</a>
    <div class="cards">
  ...
  <div>
    ...
    <?php
        $n = 0;
          For ($i=94; $i<169; $i++){
            $r = $_POST[$i];
            $res = mysqli_query($conn, "SELECT * FROM `question` WHERE id_test = '4' AND id_question = '$i'");
            $q = mysqli_fetch_assoc($res);
            $re = mysqli_query($conn, "SELECT * FROM `answer` WHERE id_test = '4' AND id_user='$id_user' AND id_question= '$i'");
